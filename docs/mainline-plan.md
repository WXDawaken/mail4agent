...
- The first standalone mailbox-language stdio interpreter shell now also exists in `mailbox_language_stdio.py`: it speaks JSON lines over stdin/stdout, supports `check` / `lower` / `run` for `protocol_schema`, `mailbox_binding`, `message_envelope`, and `handoff_event`, reuses the local protocol compile cache, and calls the same typed admin-backed runtime helpers as `client.py` instead of teaching the mailbox server to parse DSL text directly.
