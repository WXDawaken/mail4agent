...
- A new shared source parser/checker module, `mailbox_language_source.py`, now lowers a first bounded textual DSL slice into typed runtime artifacts. The current supported source grammar covers `protocol`, `mailbox`, `send`, `send text`, `spawn`, and `handoff`, while still keeping mailbox-address resolution and runtime truth outside the parser layer.
